...
def turtle4():
    mo = rg.SimpleTurtle()
    mo.pen = rg.Pen('MediumSeaGreen', 10)
    mo.right(90)
    mo.forward(200)
    mo.write_text()
    mo.speed = 10
...
def turtle5():
    cia = rg.SimpleTurtle('square')

    cia.pen = rg.Pen('khaki', 8)

    place = rg.Point(20, 40)
    ...
    for k in range(3):
        cia.right(90)
        cia.speed = 9
        cia.draw_square(size)
        size = size - 4


def turtle6():
    viga = rg.SimpleTurtle('turtle')
    viga.pen = rg.Pen('khaki', 8)
    viga.left(45)
    viga.begin_fill()
    viga.draw_circle(20)
    viga.end_fill()
